...
YSLOW.registerRule({
	...
	max_cmp: 1, // the number of components allowed before we start penalizing
	points_cmp: 15, // penalty points for each component over the maximum
	...
	var cmp = comps.length - config.max_cmp;
...
YSLOW.registerRule({
    ...
    config: {
        points: 20, // how many points to take for each component without Expires header
        howfar: 172800, // 2 days = 2 * 24 * 60 * 60 seconds, how far is far enough
        types: ['css', 'js', 'image', 'cssimage', 'flash'], // component types to be inspected for expires headers
	weight_of_size: 1, // how important is the size of components
    ...
    lint: function(doc, cset, config) {
        ...
        ts, i, offenders = [], offenders_times = [], offenders_weights = [], expiration, score,
	    offenders_time_avg = 0, offenders_total_weight = 0, offenders_weight_avg = 0,
	    offenders_weight_avg_sum = 0, offenders_avg = 0, success = false, expire_time,
        ...
        for (i = 0; i < comps.length; i++) {
        	...
        	offenders.push("(" + YSLOW.util.prettyExpiresDate(expiration) + ") " +
                           YSLOW.util.prettyAnchor(comps[i].url, comps[i].url, undefined, true, 120, undefined, comps[i].type));

	//YSLOW.util.dump(comps[i].url);
	...
	//YSLOW.util.dump(comps[i].url + '\'s size = ' + comps[i].size);

        }

	for (i = 0; i < offenders_times.length; i++) {
...
//	YSLOW.util.dump('offenders_weight_avg = ' + offenders_weight_avg);
//	YSLOW.util.dump('offenders_time_avg = ' + offenders_time_avg);

	offenders_avg = (offenders_weight_avg * config.weight_of_size + offenders_time_avg * config.weight_of_time) /
			(config.weight_of_size + config.weight_of_time);

//	YSLOW.util.dump('offenders_avg = ' + offenders_avg);
...
YSLOW.registerRule({
    ...
    config: {
        points: 25, // how many points to take out for each component not on CDN
        patterns: [ // array of regexps that match CDN-ed components
            "^http://([^\\.]*)\\.([^\\.]*)\\.yimg\\.com/[^/]*\\.yimg\\.com/.*$",
            "^http://([^\\.]*)\\.([^\\.]*)\\.yimg\\.com/[^/]*\\.yahoo\\.com/.*$",
            "^https://sec.yimg.com/",
            "^https://a248.e.akamai.net/",
            "^http://[dehlps].yimg.com/",
            "^http://(ads|cn|mail|maps|s1).yimg.com/",
            "^http://a.l.yimg.com/",
            "^http://us.(js|a)2.yimg.com/",
            "^http://yui.yahooapis.com/",
            "^http://adz.kr.yahoo.com/",
            "^http://img.yahoo.co.kr",
            "^http://img.(shopping|news|srch).yahoo.co.kr",
            "^http://pimg.kr.yahoo.com/",
            "^http://kr.img.n2o.yahoo.com/",
            "^http://s3.amazonaws.com/",
            "^http://([^\\.]*).([^\\.]*)\\.cloudfront\\.net/.*$"
        ],
        exceptions: [// array of regexps that will be treated as exception.
            "^http://chart.yahoo.com/",
            "^http://(a1|f3|f5|f3c|f5c).yahoofs.com/",       // Images for 360 and YMDB
...
YSLOW.registerRule({
    ...
    config: {
        points: 25,  // points to take out for each 404 response.
...
YSLOW.registerRule({
    ...
    config: {
        max_domains: 2, // maximum allowed domains
        				// 1 for the HTML, 1 for the CDN. should not need more than that
        points_name: 20, // the cost of each additional domain accessed by name
    ...
    lint: function(doc, cset, config) {
        ...
        var score,
            domains = YSLOW.util.getUniqueDomains(cset.components, false),
...
YSLOW.registerRule({
    ...
    config: {
        points: 25, // points to take out for each misconfigured etag
...
YSLOW.registerRule({
    ...
    config: {
        points: 40, // penalty for each unminified component
...
YSLOW.registerRule({
    ...
    config: {
        points: 100, // penalty for each duplicate
...
YSLOW.registerRule({
    ...
    id: 'yabeacons',
    //name: 'Limit image beacons',
    url: 'http://developer.yahoo.com/performance/rules.html#num_http',
    category: ['content'],

    config: {

	max_cmp: 0, // the number of components allowed before we start penalizing
	points_cmp: 20, // penalty points for each component over the maximum
	types: ['image']
	...
	var comps = cset.getComponentsByType(config.types, true, true), cmp = 0, offenders = [];
	...
	messages[messages.length] = 'This advertisement requests ' +
					YSLOW.util.plural('%num% beacon%s%', (cmp + config.max_cmp))  + '.';
	score -= cmp * config.points_cmp;
...
YSLOW.registerRule({
	...
	id: 'yaexternal',
	//name: '(JS/CSS) Inline small; cache large'
	url: 'http://developer.yahoo.com/performance/rules.html#external',
	category: ['javascript', 'css'],

	config: {
		check_inline: ['style', 'script'], // inline tags to check
		types: ['js', 'css'], // types of components to inspect
	...
	lint: function(doc, cset, config) {
		...
		var message = '', score = 100, offenders = [], i, j,
			inline_num_invalid = [], inline_offenders = [], inline_sum_invalid = 0, inline_sum_total = 0,
				inline_weight = 0, inline_score = 100, inlines,
			external_score = 100, external_comps = cset.getComponentsByType(config.types, true, false),
				external_offenders = 0, external_offenders_average_weight = 0;
        ...
        if (config.check_inline) {
            for(j = 0; j < config.check_inline.length; j++) {
                ...
                if (inlines.length > 0) {
                	for (i = 0; i < inlines.length; i++) {
                	//YSLOW.util.dump(inlines[i].innerHTML);
                	//YSLOW.util.dump(inlines[i].innerHTML.length);
                		if (inlines[i].innerHTML.length > config.threshold) {
                			inline_num_invalid[j]++;
                			inline_sum_invalid++;
                			inline_weight = (inline_weight * (inline_sum_invalid - 1) + inlines[i].innerHTML.length) /
                						inline_sum_invalid;
							offenders.push("(" + inlines[i].innerHTML.length + ") " +
								'inline ' + config.check_inline[j]);
                		}
                		inline_sum_total++;
                	}
                    message += YSLOW.util.plural('There %are% %num% inline ' +
                        (config.check_inline[j] == "style" ? 'css' : 'script%s%') +
                        ' over ' + config.threshold + ' bytes that can be made external.\n',
                        inline_num_invalid[j]);
                }
            }
        }
        ...
        // check all peeled components
        for (i = 0; i < external_comps.length; i++) {
			if (external_comps[i].size < config.threshold) {
				external_offenders_average_weight = (external_offenders_average_weight * external_offenders +
					1 - external_comps[i].size / config.threshold) / (external_offenders + 1);
				external_offenders++;
				offenders.push("(" + external_comps[i].size + ") " +
                	YSLOW.util.prettyAnchor(external_comps[i].url, external_comps[i].url, undefined, true, 120,
                	undefined, external_comps[i].type));
			}
        }

		external_score = (external_comps.length > 0 ? 100 - 100 * external_offenders_average_weight *
							external_offenders / external_comps.length : 100);
		...
		score = (inline_sum_total + external_comps.length > 0) ?
					(inline_score * inline_sum_total + external_score * external_comps.length) /
					(inline_sum_total + external_comps.length) : -1;

		//YSLOW.util.dump(inline_score);
		//YSLOW.util.dump(external_score);
		//YSLOW.util.dump(score);
...
YSLOW.registerRule({
	...
	max_cmp: 0, // the number of components allowed before we start penalizing
	points_cmp: 25, // penalty points for each component over the maximum
...
YSLOW.registerRule({
    ...
    id: 'yacookies',
    //name: 'Minimize cookies',
    url: 'http://developer.yahoo.com/performance/rules.html#cookie_size',
    category: ['cookie'],

    config: {
        points: 30, // points to take out for every KB of cookie
       	types: ['js', 'css', 'image', 'cssimage', 'flash', 'doc', 'xhr', 'iframe']
    },

    lint: function(doc, cset, config) {

        var cookie_size = 0, score = 100, i, offenders = [],
            comps = cset.getComponentsByType(config.types, true, true);
        ...
        */
        ...
        for (i = 0; i < comps.length; i++) {
            if (typeof comps[i].cookie == 'string' && comps[i].cookie.length > 0) {
            	offenders.push("(" + comps[i].cookie.length + ") " +
                	YSLOW.util.prettyAnchor(comps[i].url, comps[i].url, undefined, true, 120,
                	undefined, comps[i].type));
...
YSLOW.registerRule({
    ...
    id: 'yaexternal',
    //name: 'Make Large JS and CSS external',
    url: 'http://developer.yahoo.com/performance/rules.html#external',
    category: ['javascript', 'css'],

    config: {
        check_inline: ['style', 'script'], // inlide tags to check for minification
        threshold: 2500 // size (innerHTML.length property == size in bytes if 8bit chars. might be true regardless)
        				// after which scripts/styles should be external
    },

    lint: function(doc, components, config) {

        var i, j, inlines, message, score = 100, num_invalid = [],
            offenders = [], sum_invalid = 0, sum_total = 0, weight = 0;
                ...
                if (inlines.length > 0) {
                	...
                	//YSLOW.util.dump(inlines[i].innerHTML.length);
                		if (inlines[i].innerHTML.length > config.threshold) {
                			num_invalid[j]++;
                			sum_invalid++;
                			weight = (weight * (sum_invalid - 1) + inlines[i].innerHTML.length) /
                						sum_invalid;
                		}
                		sum_total++;
                	}
                    message = 'There %are% %num% inline ' +
                        (config.check_inline[j] == "style" ? 'css' : 'script%s%') +
                        ' over ' + config.threshold + ' bytes that can be made external.';
                    message = YSLOW.util.plural(message, num_invalid[j]);
                    offenders.push(message);
                }
            }
        }

		weight = (weight > 0 ? 1 - config.threshold / weight : 0);
		score = (sum_total > 0 ? 100 - 100 * weight * sum_invalid / sum_total : 100);

        return {
            score: (sum_total > 0 ? (score >= 0 ? score : 0) : -1),
            message: '',
            components: offenders
        };
    }
...
YSLOW.registerRule({
    ...
    id: 'yainline',
    //name: 'Inline small JS and CSS',
    url: 'http://developer.yahoo.com/performance/rules.html#external',
    category: ['javascript', 'css'],

    config: {
        types:  ['js', 'css'], // types of components to inspect for inlining
        threshold: 2500 // size (innerHTML.length property == size in bytes if 8bit chars. might be true regardless)
        				// under which scripts/styles should be inlined
    },
    ...
    lint: function(doc, cset, config) {
        ...
        var i, j, score,
            comps = cset.getComponentsByType(config.types),
            offenders = [], offenders_average_weight = 0;


        // check all peeled components
        for (i = 0; i < comps.length; i++) {
			if (comps[i].size < config.threshold) {
				offenders_average_weight = (offenders_average_weight * offenders.length + 1 - comps[i].size / config.threshold) /
											(offenders.length + 1);
				offenders.push(comps[i]);
			}
        }

	score = (comps.length > 0 ? 100 - 100 * offenders_average_weight * offenders.length / comps.length : 100);

        return {
            score: (comps.length > 0 ? (score >= 0 ? score : 0) : -1),
            message: (offenders.length > 0) ?  YSLOW.util.plural('There %are% %num% component%s% under ' + config.threshold + ' bytes that can be inlined', offenders.length) : '',
            components: offenders
        };
    }
});
...
YSLOW.registerRule({
    ...
    id: 'yaafter_onload',
    //name: 'Components that are requested after the onload event',
    url: 'http://developer.yahoo.com/performance/rules.html#num_http',
    category: ['content'],
	...
	types: ['js', 'css', 'image', 'cssimage', 'flash', 'favicon', 'doc', 'redirect', 'xhr', 'iframe']
	...
	for (i = 0; i < comps.length; i++) {
		if (comps[i].after_onload) {
			offenders.push(comps[i]);
		}
	}

	cmp = offenders.length - config.max_cmp;

	messages[messages.length] = 'This page requests ' + YSLOW.util.plural('%num% component%s%', (cmp + config.max_cmp)) +
								' after onload.';
	score -= cmp * config.points_cmp;
...
YSLOW.registerRuleset({ // ads
	...
	weights: {
		...
		yacompress:	8,
		yaredirects:	10,
		yano404:	1, //already part of yanumreq?
		yadns:		7,
		yaetags:	2,
		yaminify:	8,
		yadupes:	5,
