    ...
    exportRuleset: function(ruleset) {
        ...
        if (typeof ruleset == "object" && typeof ruleset.id == "string" && typeof ruleset.name == "string") {
            ...
            if (!("@mozilla.org/zipwriter;1" in Components.classes)) {
                return {'success': false, 'message': 'Firefox 3 is required for this feature.'};
            }

            var ext = new YSLOW.Extension(ruleset);
            ...
            var message = 'A YSLOW ruleset extension file (' + ruleset.id + '.xpi) has been created on your desktop.';
            return {'success': true, 'message': message};
        }
        } catch (err) {
            // do nothing.
        }
        return {'success': false, 'message': 'The operation failed.'};
    },
...
YSLOW.Extension = function(ruleset) {
    ...
    var zipW = Components.classes["@mozilla.org/zipwriter;1"].createInstance(Components.interfaces.nsIZipWriter);
...
YSLOW.Extension.prototype = {
    ...
    createInstallRDF: function() {
        var data = YSLOW.Exporter.INSTALL_RDF_TEMPLATE.replace(/%ruleset_id%/g, this.ruleset.id)
    ...
    createMainFile: function() {
        ...
        string = YSLOW.JSON.stringify(this.ruleset, null, 2);
        this.ruleset.custom = true;

        var builtin_rules = YSLOW.controller.getRuleset('ydefault').rules;
        var replace_func = function(key, value) {
            return (key == "lint") ? "%lint%" : value;
        };

        for (var rule_id in this.ruleset.rules) {
            if (!(rule_id in builtin_rules)) {
                var rule = YSLOW.controller.getRule(rule_id);
                var rule_string = YSLOW.JSON.stringify(rule, replace_func, 2);
                rule_string = rule_string.replace(/"%lint%"/, rule.lint.toString());
                rule_def += 'YSLOW.registerRule(\n' + rule_string + '\n);\n';
            }
        }

        var data = YSLOW.Exporter.MAIN_FILE_TEMPLATE.replace(/%ruleset_id%/g, this.ruleset.id)
                       .replace(/%ruleset_obj_string%/g, string)
                       .replace(/%rule_definition%/g, rule_def);
